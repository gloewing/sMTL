...
######################################################
setwd("~/Desktop/Research/sparseMT4")
library(dplyr)
library(latex2exp)
library(kableExtra)
library(tidyverse)
            ...
            for(k in Kvec){
                for(bl in 1:length(bVar)){
                    for(ep in 1:length(epsVecLow)){
                        ...
                        flNm <-  paste0("sprsMS_mix_LS_s_",s,"_r_",r,"_rp_0.5_numCovs_250_n_",n, ".", n, "_eps_", e1, ".", e2, "_covTyp_exponential_rho_0.5_clustDiv_10_bVar_", b, "_xVar_0_clst_", k, "_K_", k, "_bMean_0.2_0.5__bFix_TRUE_L0sseTn_sse_MSTn_multiTask_nFld_5_LSitr_50_LSspc_1_wsMeth_2_asPass_TRUE_TnIn_TRUE")
...
plt_rmse = 
    ...
    xlab(TeX('$\\mathbf{\\sigma^2_{\\epsilon}}}$')) + 
