...
# compare different K 
#######################
setwd("~/Desktop/Research/breastCancer5")
library(dplyr)
library(latex2exp)
library(kableExtra)
library(tidyverse)
...
for(p in pVec){
    ...
    for(k in Kvec){
        flNm <-  paste0("MT_breast_rand__K_", k, "_study_17_p_", p, "_totSims_100_L0sseTn_sse_rdgLmd_TRUE_MSTn_multiTask_Yscl_TRUE_rhoMax_50_nFld_5_LSitr_50_LSspc_1_fitLocal_50_Zlen_19_wsMeth_2_asPass_TRUE_TnIn_TRUE")
        ...
        if(file.exists(flNm)){
            ...
            rmseMat <- data.frame(
                                  ...
                                  "LASSO" = d$MT_lasso_low / d$MT_oseL0
            )
            ...
            suppMat <- data.frame(
                ...
                "LASSO"= d$MT_lasso_low_s - d$s_ose
