...
function BlockComIHT_inexactAS_opt_MT(; X::Matrix,
                    y,
                    rho,
                    B,
                    K,
...
########################################################
        while (iter_in <= maxIter_in)
            ...
            for k = 1:K
                r_active[:,k] = X[ :, idxInt] * B_active[:,k] - y[ :,k ]
                g_active[:, k] = (1 / n) * X[ :, idxInt]' * r_active[:,k] # gradient update for whole vector
            ...
            for k=1:K
                costAct_k = B_temp_active[2:end, k].^2 + n * cost_active[:,k] # n * cost_active[:,k] **** removed the n on 10/7/21  # scale by nVec since our objective is altered
...
########################################################
        ...
        for k = 1:K
            ...
            g[:, k] = (1 / n) * X' * r[:,k] # gradient update for whole vector
        ...
        for k=1:K
            cost_k = B_temp[2:end, k].^2 + n * cost[:,k] # scale lambda_z by nVec
