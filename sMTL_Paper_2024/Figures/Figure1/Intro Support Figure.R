...
suppSeq <- seq(2, 4 * round(r * r_p) + 1, by = 2)  # alternating sequence of covariate indices starting after common support that is "r" long
...
L0_tune <- sparseCV_iht_par(data = full,
                            ...
                            method = "MS_z3",  # this does not borrow information across the active sets
                            nfolds = nfold,
                            cvFolds = 5,
                            juliaPath = juliaPath,
...
saveRDS(betas, 
        "/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Intro Support Figure/betas")


betas <- readRDS("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Intro Support Figure/betas")
...
for(j in 1:length(unique(b$Method)) ){
    ...
    if(method == "LASSO"){
      method = "Group Lasso"
      b$Method[b$Method == "LASSO"] <- "Group Lasso"
    }else if(method == "TS-SR"){
      method = "L0L2"
      b$Method[b$Method == "TS-SR"] <- "L0L2"
    }       
    ...
    if(j == length(unique(b$Method)) ){
      ...
      assign(paste0("p", j),
             b %>% as_tibble() %>%
               ...
               geom_segment(alpha = 0.5,  aes(x=Index, xend=Index, y=0, yend=Beta)) +
               theme_bw() +
               ylab(TeX(paste0(method, ' $\\hat{\\beta}$') )) + 
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Intro Support Figure")
