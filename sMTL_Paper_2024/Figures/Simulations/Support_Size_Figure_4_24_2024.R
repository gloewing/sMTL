...
######################################################
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 5 Sims/sparseMT6")
library(dplyr)
library(latex2exp)
library(kableExtra)
library(tidyverse)
...
xVar <- c(0)  
Kvec <- c(2,4,6,8)
ls_s <- ls_f1 <- ls_supp <- ls_tp <- ls_fp <- ls_beta <- ls_coef <- vector(length = length(bVar) * length(Kvec), "list")
          ...
          for(bl in 1:length(bVar)){
            for(ep in 1:length(epsVecLow)){
              ...
              if(file.exists(flNm)){
                ...
                sMat <- data.frame("LASSO" = d$s_lassoMrg,
                                   oseL0 = d$s_ose,
                                   "CS+Bbar" = d$s_MS3,
                                   "Zbar+L2" = d$s_MS2,
                                   "CS+L2" = d$s_MS1,
