...
#######################################################
setwd("/Users/loewingergc/Desktop/Research/smtl_sims8")
library(dplyr)
library(latex2exp)
library(kableExtra)
library(tidyverse)
                    ...
                    for(bl in 1:length(bVar)){
                        for(ep in 1:length(epsVecLow)){
                            ...
                            if(file.exists(flNm)){
                                ...
                                names(f1) <- names(suppMat)[names(suppMat) != "Trace"] 
...
levs2 <- grp_incl2 <- c("GL", "CS+L2", "CS+Bbar", "Zbar+L2", "Zbar+Bbar", "cMCP")
...
for(eRho in rVec){
  ...
  plt_r = dat %>% tibble %>%  
      dplyr::filter(q <= 20,
                    ...
                    value != 0,
                    k %in% c(4) ) %>%
      dplyr::group_by(b,k,n,e1,q, rho, key) %>% 
      ...
      facet_grid(n ~ rho, scales = "free_y") + 
      geom_line(size = 0.4) +
      ...
      theme( plot.title = element_text(hjust = 0.5, color="black", size=rel(1.5), face="bold"),
             ...
             strip.text.x = element_text(face="bold", color="black", size = rel(1.5)) ) + guides(fill= guide_legend(title="Method"))  
  ...
  ggsave( paste0("multiTaskRMSE_q_", eRho, ".pdf"),
          plot = plt_r,
          width = 12,
          height = 8)
  
  if(eRho == 0.5)     plt_rmse <- plt_r
  assign(paste0("plt_r", cnt), plt_r)
  cnt <- cnt + 1
}

f1_rmse <- ggarrange(plt_r1, plt_r2, plt_r3, ncol=1, nrow=3, common.legend = TRUE, legend="bottom")

setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskRMSE_joined.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
for(eRho in rVec){
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskRMSE_q_", eRho, "_supp.pdf"),
          plot = plt_r,
          width = 12,
          height = 8)
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskRMSE_joined_supp.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
for(eRho in rVec){
  ...
  plt_f1 = 
      ...
      dplyr::summarize(my_mean = mean(value, na.rm = TRUE),
                       sims = n(),
                       sd = sd(value, na.rm = TRUE), se = sd/sqrt(sims) ) %>% 
      arrange(b, k,  q, rho, n) %>%
      rename("Method"="key" ) %>%
      ggplot(aes( y = my_mean, x = 10 / q, fill = Method, color = Method )) +
      facet_grid(n ~ rho, scales = "free_y") + 
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskF1_q_", eRho, ".pdf"),
          plot = plt_f1,
          width = 12,
          height = 8)
  
  if(eRho == 0.5)     plt_f <- plt_f1
  
  assign(paste0("plt_r", cnt), plt_f1)
  cnt <- cnt + 1
}
...
plt_f = 
  ...
  dplyr::filter(q <= 20,
                ...
                n %in% c(50,100),
                r_exp == 0.5,
                key %in% grp_incl[grp_incl != "cMCP"], # performance so bad skews axis
                k %in% c(4) ) %>%
  dplyr::group_by(b,k,n,e1,q, rho, key) %>% 
  dplyr::summarize(my_mean = mean(value, na.rm = TRUE),
                   sims = n(),
  ...
  rename("Method"="key" ) %>%
  ggplot(aes( y = my_mean, x = 10 / q, fill = Method, color = Method )) +
  facet_grid(n ~ rho, scales = "free_y") + # , scales = "free_y"
  geom_line(size = 0.4) +
  geom_point(aes(shape = Method))+
  geom_errorbar(aes(ymin=my_mean-se, ymax=my_mean+se), linewidth = 0.35) +#,
  geom_hline(yintercept=1, 
...
f1_rmse <- ggarrange(plt_rmse, plt_f, ncol=2, nrow=1, common.legend = TRUE, legend="bottom")
...
for(eRho in rVec){
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskF1_q_", eRho, "_supp.pdf"),
          plot = plt_f1,
          width = 12,
          height = 8)
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskF1_joined_supp.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
for(eRho in rVec){
  ...
  plt_coef = dat %>% tibble %>%  
      dplyr::filter(q <= 20,
                    ...
                    key %in% grp_incl,
                    r_exp == eRho,
                    value <= 29, # remove single outlier
                    k %in% c(4) ) %>%
      dplyr::group_by(b,k,n,e1,q, rho, key) %>% 
      dplyr::summarize(my_mean = mean(value, na.rm=TRUE),
                       sims = n(),
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskCoef_q_", eRho, ".pdf"),
          plot = plt_coef,
          width = 12,
          height = 8)
  
  assign(paste0("plt_r", cnt), plt_coef)
  cnt <- cnt + 1
}
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskCoef_joined.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
for(eRho in rVec){
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskCoef_q_", eRho, "_supp.pdf"),
          plot = plt_coef,
          width = 12,
          height = 8)
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskCoef_joined_supp.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)

####################################################################################################
# RMSE unadjusted (not adjusted by L0L2)
####################################################################################################
# factors
dat <- do.call(rbind, ls_beta2)
dat$b <- as.factor(dat$b)
dat$k <- as.factor(dat$k)
dat$s <- as.factor(dat$s)
dat$r_exp <- as.factor(dat$r_exp)
...
for(eRho in rVec){
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskRMSE_unadjust_q_", eRho, ".pdf"),
          plot = plt_r,
          width = 12,
          height = 8)
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskRMSE_unadjust_joined.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
for(eRho in rVec){
  ...
  setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
  ggsave( paste0("multiTaskRMSE_unadjust_q_", eRho, "_supp.pdf"),
          plot = plt_r,
          width = 12,
          height = 8)
...
setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTaskRMSE_unadjust_joined_supp.pdf",
        plot = f1_rmse,
        width = 16,
        height = 21)
...
# combined 
#-------------------------------
f1_supp <- ggarrange(plt_rmse, plt_f, plt_fp, plt_tp, ncol=2, nrow=2, common.legend = TRUE, legend="bottom")

setwd("/Users/loewingergc/Desktop/NIMH Research/Sparse Multi-Study/Figures/Figure 6 - Sims support heterogeneity (q) vs performance/Resubmission/Final/Figures")
ggsave( "multiTask_combined_q.pdf",
        plot = f1_supp,
        width = 16,
        height = 20)
