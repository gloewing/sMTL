...
2. Three $\texttt{Julia}$ packages are required ($\texttt{TSVD}$, $~\texttt{LinearAlgebra}$ and $~\texttt{Statistics}$). We include a function to automatically install these if they are not already. If $\texttt{Julia}$ is installed but the packages are not, the function can install them without re-installing $\texttt{Julia}$. If you ran the code for 1. above, you can ignore the rest of the set-up (2-3) and skip to fitting functions.
