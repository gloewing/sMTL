...
To fit a solutions for the Common Support Multi-Task model with 5 non-zero coefficients (i.e., $s=5$) and a Ridge penalty $\lambda_1 = 0.001$, we use the `smtl` function and then print out the coefficients stored in $\texttt{fit}$. Notice here the $y$ object in $\texttt{R}$ is a matrix, and that we do not include the `study=` syntax that we use for Multi-Task learning or Domain Generalization. Instead the columns of the $y$ object are treated as separate ``labels'' (akin to separate tasks). Otherwise the syntax is the exact same.
...
# References
Gabriel Loewinger, Kayhan Behdin, Kenneth Kishida, Giovanni Parmigiani and Rahul Mazumder. [Multi-Task Learning for Sparsity Pattern Heterogeneity: A Discrete Optimization Approach](https://scholar.google.com/scholar_url?url=https://arxiv.org/pdf/1707.08114&hl=en&sa=X&ei=6IbIYr7xMo-5yQTW04rIBw&scisig=AAGBfm12ecsT1Wt4UCLfp0fvnw_iHIjZ2w&oi=scholarr). arXiv: (2022).
